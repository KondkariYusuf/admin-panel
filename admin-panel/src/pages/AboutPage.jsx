...
//   // files state maps
//   const [galleryFiles, setGalleryFiles] = useState([]); // array of File | null, maps to galleryImages
//   const [mediaFiles, setMediaFiles] = useState([]); // single file expected
//   const [teamFiles, setTeamFiles] = useState([]); // array of File | null mapping to members
//   const [loading, setLoading] = useState(false);
//   const [message, setMessage] = useState("");

//   // ---------- helpers ----------
//   const setPageTitle = (patch) => setModel((m) => ({ ...m, pageTitle: { ...m.pageTitle, ...patch } }));
//   const setAboutArea = (patch) => setModel((m) => ({ ...m, aboutArea: { ...m.aboutArea, ...patch } }));
//   const setApproachSection = (patch) => setModel((m) => ({ ...m, approachSection: { ...m.approachSection, ...patch } }));
//   const setInfoSection = (patch) => setModel((m) => ({ ...m, infoSection: { ...m.infoSection, ...patch } }));
//   const setMediaSection = (patch) => setModel((m) => ({ ...m, mediaSection: { ...m.mediaSection, ...patch } }));
//   const setAwardsSection = (patch) => setModel((m) => ({ ...m, awardsSection: { ...m.awardsSection, ...patch } }));
//   const setTeamSection = (patch) => setModel((m) => ({ ...m, teamSection: { ...m.teamSection, ...patch } }));

//   // aboutArea: paragraphs management
//   const addParagraph = () => setAboutArea({ paragraphs: [...model.aboutArea.paragraphs, ""] });
//   const updateParagraph = (i, text) => {
//     const next = model.aboutArea.paragraphs.map((p, idx) => (idx === i ? text : p));
//     setAboutArea({ paragraphs: next });
//   };
//   const removeParagraph = (i) => setAboutArea({ paragraphs: model.aboutArea.paragraphs.filter((_, idx) => idx !== i) });

//   // aboutArea: gallery images manipulation (imageUrl + alt). We keep galleryFiles mapped by index for uploads
//   const handleGalleryFiles = (files) => {
//     // replace entire galleryFiles with chosen files (ImageUpload returns File[])
//     setGalleryFiles(files);
//     // if files provided, clear corresponding imageUrl entries to signal upload
//     if (files && files.length) {
//       const nextGallery = model.aboutArea.galleryImages.map((g, idx) => ({ ...g, imageUrl: "" }));
//       setAboutArea({ galleryImages: nextGallery });
//     }
//   };

//   const updateGalleryMeta = (i, patch) => {
//     const next = model.aboutArea.galleryImages.map((g, idx) => (idx === i ? { ...g, ...patch } : g));
//     setAboutArea({ galleryImages: next });
//   };

//   // approachSection approaches items use TagInput
//   const setApproachItems = (approachIndex, items) => {
//     const next = model.approachSection.approaches.map((a, idx) => idx === approachIndex ? { ...a, items } : a);
//     setApproachSection({ approaches: next });
//   };
//   const updateApproach = (i, patch) => {
//     const next = model.approachSection.approaches.map((a, idx) => idx === i ? { ...a, ...patch } : a);
//     setApproachSection({ approaches: next });
//   };
//   const addApproach = () => {
//     setApproachSection({ approaches: [...model.approachSection.approaches, { title: "New approach", showShape: false, items: [] }] });
//   };
//   const removeApproach = (i) => setApproachSection({ approaches: model.approachSection.approaches.filter((_, idx) => idx !== i) });

//   // awards: add/remove categories and award items
//   const addAwardCategory = () => {
//     setAwardsSection({ awards: [...(model.awardsSection.awards || []), { category: "New Category", items: [] }] });
//   };
//   const updateAwardCategory = (idx, patch) => {
//     const next = (model.awardsSection.awards || []).map((a, i) => i === idx ? { ...a, ...patch } : a);
//     setAwardsSection({ awards: next });
//   };
//   const removeAwardCategory = (idx) => setAwardsSection({ awards: (model.awardsSection.awards || []).filter((_, i) => i !== idx) });

//   const addAwardItem = (catIdx) => {
//     const copy = [...(model.awardsSection.awards || [])];
//     copy[catIdx].items = [...(copy[catIdx].items || []), { title: "New Award", year: new Date().getFullYear().toString() }];
//     setAwardsSection({ awards: copy });
//   };
//   const updateAwardItem = (catIdx, itemIdx, patch) => {
//     const copy = [...(model.awardsSection.awards || [])];
//     copy[catIdx].items = copy[catIdx].items.map((it, i) => i === itemIdx ? { ...it, ...patch } : it);
//     setAwardsSection({ awards: copy });
//   };
...
//   // team members: add/remove/update + file mapping via teamFiles
//   const addMember = () => {
//     setTeamSection({ members: [...model.teamSection.members, { name: "New Member", post: "Role", imageUrl: "" }] });
//     setTeamFiles((p) => [...p, null]);
//   };
//   const updateMember = (idx, patch) => {
//     const next = model.teamSection.members.map((m, i) => i === idx ? { ...m, ...patch } : m);
//     setTeamSection({ members: next });
//   };
//   const removeMember = (idx) => {
//     setTeamSection({ members: model.teamSection.members.filter((_, i) => i !== idx) });
...
//       return copy;
//     });
//     // clear imageUrl if a file chosen
...
//     setMediaFiles(files);
//     if (files && files.length) setMediaSection({ mediaImage: { imageUrl: "" } });
//   };

//   // infoSection stats editing helpers
//   const updateStat = (i, patch) => {
//     const next = model.infoSection.stats.map((s, idx) => idx === i ? { ...s, ...patch } : s);
//     setInfoSection({ stats: next });
//   };

//   // submit: package JSON payload and uploaded files
...
//       // set gallery imageUrl fields to "" if uploaded files exist (we'll upload them)
//       payload.aboutArea.galleryImages = payload.aboutArea.galleryImages.map((g, idx) => ({
//         ...g,
//         imageUrl: galleryFiles[idx] ? "" : (g.imageUrl || "")
//       }));

//       // mediaSection mediaImage
//       payload.mediaSection = { ...payload.mediaSection, mediaImage: { ...payload.mediaSection.mediaImage, imageUrl: mediaFiles[0] ? "" : (payload.mediaSection.mediaImage.imageUrl || "") } };

//       // team images
//       payload.teamSection = { ...payload.teamSection, members: payload.teamSection.members.map((m, idx) => ({ ...m, imageUrl: teamFiles[idx] ? "" : (m.imageUrl || "") })) };
...
//       // append media image (single)
//       if (mediaFiles && mediaFiles.length) fd.append("mediaImage", mediaFiles[0], mediaFiles[0].name);
...
//       const res = await api.post("/admin/about", fd, {
...
//     <div className="">
//         <Navbar/>
//       <h1 className="text-2xl font-semibold mb-4 p-6 max-w-6xl mx-auto">Edit About Page</h1>

//       {message && <div className="mb-4 px-4 py-2 rounded bg-white/6">{message}</div>}
...
//           <label className="text-sm text-[var(--muted)]">Info List (tags)</label>
//           <TagInput value={model.aboutArea.infoList.items} onChange={(arr) => setAboutArea({ infoList: { items: arr } })} />
...
//                   />
//                   <button type="button" onClick={() => removeParagraph(i)} className="px-3 py-2 rounded bg-red-600">Remove</button>
//                 </div>
//               ))}
//               <button type="button" onClick={addParagraph} className="px-3 py-2 rounded bg-white/6">+ Add paragraph</button>
//             </div>
//           </div>

//           <div className="mt-4">
//             <label className="text-sm text-[var(--muted)]">Gallery Images</label>
//             {/* Use ImageUpload for gallery; it returns File[] */}
//             <ImageUpload value={galleryFiles} onChange={handleGalleryFiles} maxFiles={8} />
//             <div className="mt-2 space-y-2">
//               {model.aboutArea.galleryImages.map((g, idx) => (
//                 <div key={idx} className="flex items-center gap-3">
//                   <input
//                     value={g.imageUrl}
//                     onChange={(e) => updateGalleryMeta(idx, { imageUrl: e.target.value })}
//                     placeholder="Existing image path (or leave blank if uploading)"
//                     className="flex-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//                   />
//                   <input
//                     value={g.alt}
//                     onChange={(e) => updateGalleryMeta(idx, { alt: e.target.value })}
...
//             onChange={(e) => setApproachSection({ subtitle: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <label className="text-sm text-[var(--muted)] mt-3">Heading</label>
//           <input
//             value={model.approachSection.heading}
//             onChange={(e) => setApproachSection({ heading: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <div className="mt-3 space-y-3">
//             {model.approachSection.approaches.map((a, i) => (
//               <div key={i} className="p-3 rounded bg-[#0b1220] border border-white/8">
...
//                       type="checkbox"
//                       checked={a.showShape}
//                       onChange={(e) => updateApproach(i, { showShape: e.target.checked })}
//                     />
//                     Show shape
//                   </label>
//                 </div>
...
//                 <div className="mt-2">
//                   <label className="text-sm text-[var(--muted)]">Items (tags)</label>
//                   <TagInput value={a.items || []} onChange={(items) => setApproachItems(i, items)} />
//                 </div>

//                 <div className="mt-2 flex gap-2">
//                   <button type="button" onClick={() => removeApproach(i)} className="px-3 py-1 rounded bg-red-600">Remove</button>
//                 </div>
//               </div>
//             ))}

//             <button type="button" onClick={addApproach} className="px-3 py-2 rounded bg-white/6">+ Add approach</button>
//           </div>
//         </section>
...
//             onChange={(e) => setInfoSection({ subtitle: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />
...
//             onChange={(e) => setInfoSection({ heading: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />
...
//             onChange={(e) => setMediaSection({ heading: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <label className="text-sm text-[var(--muted)] mt-3">Text</label>
//           <textarea
//             rows={3}
//             value={model.mediaSection.text}
//             onChange={(e) => setMediaSection({ text: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <div className="mt-3">
//             <label className="text-sm text-[var(--muted)]">Existing image path</label>
//             <input
//               value={model.mediaSection.mediaImage.imageUrl}
//               onChange={(e) => setMediaSection({ mediaImage: { ...model.mediaSection.mediaImage, imageUrl: e.target.value } })}
//               className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//             />
//             <div className="mt-2">
//               <ImageUpload value={mediaFiles} onChange={handleMediaFiles} maxFiles={1} maxSizeMB={5} />
//             </div>
//           </div>
//         </section>
...
//             <button type="button" onClick={addAwardCategory} className="px-3 py-1 rounded bg-white/6">+ Category</button>
//           </div>

//           <label className="text-sm text-[var(--muted)] mt-3">Heading</label>
//           <input
//             value={model.awardsSection.heading}
//             onChange={(e) => setAwardsSection({ heading: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <div className="mt-3 space-y-3">
//             {(model.awardsSection.awards || []).map((cat, ci) => (
//               <div key={ci} className="p-3 rounded bg-[#0b1220] border border-white/8">
//                 <div className="flex items-center justify-between">
//                   <input
//                     value={cat.category}
//                     onChange={(e) => updateAwardCategory(ci, { category: e.target.value })}
//                     className="px-3 py-2 rounded bg-[#071028] border border-white/8"
//                   />
//                   <button type="button" onClick={() => removeAwardCategory(ci)} className="px-3 py-1 rounded bg-red-600">Remove</button>
//                 </div>
...
//                       <input
//                         value={it.title}
//                         onChange={(e) => updateAwardItem(ci, ii, { title: e.target.value })}
//                         className="flex-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//                       />
//                       <input
//                         value={it.year}
//                         onChange={(e) => updateAwardItem(ci, ii, { year: e.target.value })}
//                         className="w-28 px-3 py-2 rounded bg-[#071028] border border-white/8"
//                       />
//                       <button type="button" onClick={() => removeAwardItem(ci, ii)} className="px-3 py-1 rounded bg-red-600">Remove</button>
//                     </div>
//                   ))}
//                   <button type="button" onClick={() => addAwardItem(ci)} className="px-3 py-1 rounded bg-white/6">+ Add award</button>
//                 </div>
//               </div>
//             ))}
//           </div>
...
//               <button type="button" onClick={addMember} className="px-3 py-1 rounded bg-white/6">+ Member</button>
//             </div>
//           </div>
...
//             onChange={(e) => setTeamSection({ heading: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <label className="text-sm text-[var(--muted)] mt-3">Description</label>
//           <textarea
//             rows={2}
//             value={model.teamSection.description}
//             onChange={(e) => setTeamSection({ description: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />
...
//                     <input value={m.name} onChange={(e) => updateMember(i, { name: e.target.value })} className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8" />
//                   </div>

//                   <div>
//                     <label className="text-sm text-[var(--muted)]">Post</label>
//                     <input value={m.post} onChange={(e) => updateMember(i, { post: e.target.value })} className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8" />
//                   </div>
...
//                       maxSizeMB={5}
//                     />
//                     <div className="text-xs text-[var(--muted)] mt-2">Current: {m.imageUrl || "(upload to replace)"}</div>
//                   </div>
//                 </div>

//                 <div className="mt-2 flex justify-end">
//                   <button type="button" onClick={() => removeMember(i)} className="px-3 py-1 rounded bg-red-600">Remove</button>
//                 </div>
//               </div>
//             ))}
//           </div>
