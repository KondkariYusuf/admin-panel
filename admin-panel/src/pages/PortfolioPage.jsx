 ...
 *  GET    /portfolio/:portfolioId/projects/:projectId
 *  PUT    /portfolio/:portfolioId/projects/:projectId
 *  DELETE /portfolio/:portfolioId/projects/:projectId
 *
 * Notes:
 * - Controller shown earlier expects JSON bodies for portfolio & project endpoints.
 * - File uploads are NOT part of those endpoints. If you want to upload local files,
 *   implement a dedicated upload endpoint (e.g., POST /upload) that returns a hosted image URL,
 *   then call uploadFile(file) below and store returned url into project's imageUrl.
 */
...
export default function PortfolioPage() {
  ...
  // -------------------------
  // Helper: upload a file to your upload endpoint
  // -------------------------
  // IMPORTANT:
  // Your controllers above do not provide a file upload endpoint.
  // If you add one (e.g., POST /upload -> returns { url }), use this function.
  // If you don't have an upload endpoint, you must provide imageUrl strings (CDN) manually.
  async function uploadFile(file) {
    if (!file) return null;
    // Example: POST /upload (you must implement server-side)
    // const fd = new FormData();
    // fd.append("file", file, file.name);
    // const res = await api.post("/upload", fd, { headers: { "Content-Type": "multipart/form-data" } });
    // return res.data?.url || null;

    // If you don't have /upload implemention, throw so caller can fallback
    throw new Error(
  ...
  // -------------------------
  // Create or update entire portfolio (recentWork + projects)
  // - Uses POST /portfolio to create when no portfolio exists
  // - Uses PUT  /portfolio/:id to update when exists
  // Note: this sends JSON only. If you want file upload, implement /upload and use uploadFile above to upload images first and set imageUrl accordingly.
  // -------------------------
  const savePortfolio = async (e) => {
    ...
    try {
      // Before send: if there are projectImages selected and you have upload endpoint,
      // upload them and fill imageUrl for payload. Otherwise leave imageUrl as-is (must be provided)
      const payload = JSON.parse(JSON.stringify(model));

      // Attempt to upload projectImages if upload endpoint exists.
      // This code tries to upload, but will throw if uploadFile isn't implemented.
  ...
  // -------------------------
  // Add a single project using controller endpoint:
  // POST /portfolio/:id/projects
  // body: { title, year, category, imageUrl }
  // -------------------------
  const addProjectRemote = async (index) => {
    ...
    setMessage("");
    try {
      // If a local image file is selected for this project, you must upload it and obtain imageUrl first.
      ...
      // After adding remotely, server sends back the added project.
      // We'll replace local project with returned one (which includes _id).
      if (res.data?.project) {
  ...
  // -------------------------
  // Update a single project via API:
  // PUT /portfolio/:portfolioId/projects/:projectId
  // Accepts only allowed fields: title, year, category, imageUrl (controller)
  // -------------------------
  ...
  // -------------------------
  // Delete entire portfolio (server)
  // DELETE /portfolio/:id
  // -------------------------
