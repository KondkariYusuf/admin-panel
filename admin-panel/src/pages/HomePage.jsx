...
//  * - Sends multipart/form-data to backend (adjust endpoint)
//  */
...
//   const [videoFiles, setVideoFiles] = useState([]); // will hold File[] for video upload
//   const [parallaxFiles, setParallaxFiles] = useState([]); // image upload for parallax (single expected)
//   const [serviceImages, setServiceImages] = useState([]); // array maps to services by index: File or null
...
//   // ---------- services management (array of objects) ----------
//   const addService = () => {
//     setServiceSection({
//       services: [
...
//     const next = model.serviceSection.services.map((s, i) => (i === index ? { ...s, ...patch } : s));
...
//   // ---------- funFact items ----------
//   const addFunFact = () => {
//     setFunFactSection({ items: [...model.funFactSection.items, { value: "", text: "" }] });
//   };
//   const updateFunFact = (i, patch) => {
//     const next = model.funFactSection.items.map((it, idx) => (idx === i ? { ...it, ...patch } : it));
...
//     // if user selected a file, update model parallaxImage.imageUrl to empty or keep existing
//     if (files.length) setParallaxImage({ imageUrl: "" });
//   };

//   // ---------- service images handler (maps by index) ----------
//   const handleServiceImageChange = (index, files) => {
//     // store file for that index
//     setServiceImages((prev) => {
//       const copy = [...prev];
//       copy[index] = files && files.length ? files[0] : null;
//       return copy;
//     });
//     // if provided file, set imageUrl to empty to indicate we'll upload
...
//       // append JSON fields as a single 'payload' JSON for convenience
//       // but strip out file path fields we will upload separately
//       const payload = JSON.parse(JSON.stringify(model));

//       // Remove fields that will be sent as files (we will upload actual files)
//       // parallaxImage.imageUrl, service.imageUrl, video.videoUrl may be replaced by uploads
//       payload.parallaxImage = { ...payload.parallaxImage, imageUrl: "" };

//       payload.serviceSection = {
//         ...payload.serviceSection,
//         services: payload.serviceSection.services.map((s) => ({ ...s, imageUrl: "" })),
//       };
...
//     <div className="">
//         <Navbar />
//       <h1 className="text-2xl font-semibold mb-4 max-w-5xl mx-auto p-6">Edit Home Page</h1>
...
//             <div>
//               <label className="block text-sm text-[var(--muted)]">Satisfied Clients %</label>
//               <input
//                 type="number"
//                 value={model.hero.satisfiedClientsPercent}
//                 onChange={(e) => setHero({ satisfiedClientsPercent: Number(e.target.value) })}
//                 className="w-full px-3 py-2 mt-1 rounded bg-[#071028] border border-white/8"
//               />
//             </div>

//             <div>
//               <label className="block text-sm text-[var(--muted)]">Projects Count</label>
//               <input
//                 type="number"
//                 value={model.hero.projectsCount}
//                 onChange={(e) => setHero({ projectsCount: Number(e.target.value) })}
//                 className="w-full px-3 py-2 mt-1 rounded bg-[#071028] border border-white/8"
//               />
//             </div>

//             <div>
//               <label className="block text-sm text-[var(--muted)]">Countries Count</label>
//               <input
//                 type="number"
//                 value={model.hero.countriesCount}
//                 onChange={(e) => setHero({ countriesCount: Number(e.target.value) })}
//                 className="w-full px-3 py-2 mt-1 rounded bg-[#071028] border border-white/8"
//               />
//             </div>
//           </div>
...
//           <div className="mb-3">
//             <label className="block text-sm text-[var(--muted)]">Existing Video URL (optional)</label>
//             <input
//               type="text"
...
//               placeholder="https://..."
//             />
//             <div className="text-xs text-[var(--muted)] mt-1">Or upload a new video below (will replace URL).</div>
//           </div>
...
//             <button type="button" onClick={addService} className="px-3 py-1 rounded bg-white/6 text-sm">
//               + Add service
//             </button>
//           </div>
...
//                     <input
//                       value={s.number}
//                       onChange={(e) => updateService(idx, { number: e.target.value })}
...
//                   <div>
//                     <label className="text-sm text-[var(--muted)]">Details URL</label>
//                     <input
//                       value={s.detailsUrl}
//                       onChange={(e) => updateService(idx, { detailsUrl: e.target.value })}
//                       className="w-full px-3 py-2 mt-1 rounded bg-[#071028] border border-white/8"
//                     />
//                   </div>
//                 </div>
...
//                 <div className="mt-3">
//                   <label className="text-sm text-[var(--muted)]">List (tags)</label>
//                   <TagInput value={s.list || []} onChange={(arr) => setServiceList(idx, arr)} />
//                 </div>

//                 <div className="mt-3">
//                   <label className="text-sm text-[var(--muted)]">Service Image</label>
//                   {/* We use ImageUpload but restrict to single file for service images */}
//                   <ImageUpload
...
//             <button type="button" onClick={addFunFact} className="px-3 py-1 rounded bg-white/6 text-sm">
//               + Add item
//             </button>
//           </div>
...
//             onChange={(e) => setClientSection({ descriptionText: e.target.value })}
//             className="w-full mt-1 px-3 py-2 rounded bg-[#071028] border border-white/8"
//           />

//           <div className="mt-3">
//             <label className="text-sm text-[var(--muted)]">Company Names (tags)</label>
//             <TagInput value={model.clientSection.companyNames} onChange={setClientCompanies} />
//           </div>
//         </section>
...
//           <div className="mt-3">
//             <div className="text-sm text-[var(--muted)]">Existing image path: {model.parallaxImage.imageUrl || "—"}</div>
//             <ImageUpload value={parallaxFiles} onChange={handleParallaxFiles} maxFiles={1} maxSizeMB={5} />
//           </div>
//         </section>
